...
\begin{equation}\label{eqn:post_joint_factorized}
  ...
  p(\boldsymbol{\Theta}, \mathcal{X},
    ...
    p(\boldsymbol{\Theta})p(\mathbf{x}_0)p(\mathcal{L})
...
In order to solve \eqref{eqn:map_estimator}, we refine our problem by defining
a \emph{motion} and \emph{observation} model
