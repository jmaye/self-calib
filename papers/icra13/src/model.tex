...
Starting with an initial guess $p(\mathbf{\Theta})$ at time $t$, we collect a
measurement \emph{batch} $\mathcal{D}^{new}=\{\mathbf{u}_{t:t+k},
\mathbf{z}_{t_{1:N}:{t+k}_{1:N}}\}$ during $k$ timesteps and compute
$p(\mathbf{\Theta}\mid\mathcal{D}^{new})$ using Gauss-Newton method with TQR
updates. In a second step, if $I(\mathbf{\Theta};\mathcal{D}^{new})$ is larger
than a user-defined threshold $\theta$,
$\mathcal{D}^{new}$ is added to the set of the informative measurements
$\mathcal{D}^{info}$ and the estimate of $\mathbf{\Theta}$ is updated.

\begin{algorithm}[ht]
\caption{detectObjects()}
\label{alg:calibration}
\dontprintsemicolon
\SetKw{KwAnd}{and}
\SetKwComment{Comment}{// }{}
\SetKwFunction{updateStateModel}{updateStateModel}
\SetKwFunction{nearestNeighbor}{nearestNeighbor}
\KwIn{Set of 3D range observations}
\KwIn{State model parameters}
\KwIn{Clustering model parameters}
\KwOut{Object mapping}
\KwOut{Updated state model parameters}
\vskip 0.2cm
%\Comment*[h]{State estimation} \;
%$\Z_{dyn} \gets \varnothing$ \;
%\ForEach {$z_i \in \Z_{1:N}$}{
%  $\Theta_\x' \gets$ \updateStateModel{$\Theta_\x$, $z_i$} \;
%  Evaluate $\cpr{x_i = 1}{z_i, \Theta_\x'} $ using
%    Eqn.~\ref{eq:state_model} and \cite{Stauffer00Learning} \;
%  \If {$\cpr{x_i = 1}{z_i, \Theta_\x'} \geq p_{dyn}$}{
%    $\Z_{dyn} \gets \Z_{dyn} \cup \{z_i\}$ \;
%  }
%}
%\Comment*[h]{Clustering} \;
%\ForEach {$z_i \in \Z_{dyn}$}{
%  $c(z_i) \gets l_i$ \Comment*[f]{Initialize labels} \;
%}
%\While {$\Z_{dyn} \neq \varnothing$}{
%  \ForEach {$z_i \in \Z_{dyn}$}{
%    $\Z_{dyn} \gets \Z_{dyn} \setminus \{z_i\}$ \;
%    $z_j \gets$ \nearestNeighbor{$\Z_{dyn}$, $z_i$} \;
%    \If {$c(z_i) \neq c(z_j)$ \KwAnd
%        $\|t_c(z_i), t_c(z_j))\| \leq \sigma_{c(z_j)}$}{
%      \Comment*[h]{Merge clusters} \;
%      $c(z_i) \gets c(z_j)$ \;
%      $\Z_{dyn} \gets \Z_{dyn} \cup \{z_i\}$ \;
%    }
%  }
