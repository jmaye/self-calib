...
According to~\cite{durrantwhyte06simultaneous}, the full joint posterior on the
right-hand side of \eqref{eqn:post_calib} may further be factorized into
...
whose mean, $\boldsymbol{\mu}_{\boldsymbol{\Theta}\mathcal{X}\mathcal{L}}$, and
covariance, $\boldsymbol{\Sigma}_{\boldsymbol{\Theta}\mathcal{X}\mathcal{L}}$,
have to be estimated. To this end, we first derive a Maximum a Posteriori (MAP)
...
with added Gaussian noise. In the following, we will coin $\mathbf{A}$
