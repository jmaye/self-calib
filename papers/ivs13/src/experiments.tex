...
comparable to the LS estimator, while using fewer measurements. Although being
fed by an initial prior, the EKF proves to be inferior to batch methods.

\begin{figure*}[t]
\begin{center}
\includegraphics[]{fig/sine-wave-dx.eps}
\includegraphics[]{fig/sine-wave-dy.eps}
\includegraphics[]{fig/sine-wave-psi.eps}
\caption{Comparison of a least squares (red), EKF (green), and our TQR-MI (blue)
  ...
  simulations are performed and the computed calibration parameters ($\delta_x$,
  $\delta_y$, and $\psi$) are reported in boxplot form. The dashed blue lines
  represent the ground truth calibration parameters.}
