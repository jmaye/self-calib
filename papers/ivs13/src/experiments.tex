...
some representative examples. We have first driven the robot along a straight
...
matrix, 5 values are exactly $0$, i.e., a structural rank deficiency. By adding
noise to the system, only 2 singular values remain at $0$ for the same problem.
From the integrated odometry path in Fig.~\ref{fig:straight-path}, the
system believes that the calibration parameters are indeed observable and will
thus wrongly optimize. This justifies the need for our TSVD/TQR method.
