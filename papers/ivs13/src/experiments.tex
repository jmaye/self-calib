...
%simplicity of the setup, our algorithm is flexible enough to cope with
%more complex scenarios, e.g., multiple heterogeneous sensors or 3D
%motion.
...
setup, we have used a batch size of $k=100$ (ten seconds of data). Concerning the MI threshold, a
...
structurally unobservable. Indeed, it has 5 unobservable parameters, 3 corresponding to the global pose of the map and trajectory (as no global measurements or prior are included) and 2 for the calibration offset variables
...
the Woods Dataset'' provided with the courtesy of Tim Barfoot \cite{tong_crv12}. This dataset
...
least-squares estimator that exploits all the measurements. Our TQR-MI algorithm
performs the optimization with $K=1209$ out of $12609$ timesteps. Although the
calibration parameters are the same magnitude, the higher variances stem
from the fewer number of considered measurements.
