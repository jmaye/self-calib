...
\noindent Thus, instead of directly solving \eqref{eqn:dx_update} which requires
a significant amount of computation and introduces numerical errors, we can take
...
combination of the first and the last row) with added Gaussian noise. In the
following, we call $\mathbf{A}$ a \emph{numerically rank-deficient} matrix. The
issue of rank deficiency often appears in robotics state estimation and is
related to the problem of \emph{observability} in control
theory~\cite{jauffret07observability}.
...
\begin{equation}\label{eqn:scaling_matrix}
  \begin{aligned}
  \mathbf{G} &= \diag(\frac{1}{||\mathbf{L}\mathbf{H}(:,1)||},\cdots,
    \frac{1}{||\mathbf{L}\mathbf{H}(:,n)||}),
  \end{aligned}
\end{equation}
...
data structures. The memory and computational costs are then $\mathcal{O}(n)$,
where $n$ is the number of timesteps considered for optimization.
