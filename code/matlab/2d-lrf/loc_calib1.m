...
% based on Tim Barfoot's example. For the sake of simplicity, landmark's
% positions are not estimated here.
...
% matrices allocation
% jacobian matrix
H = sparse(3 * steps + rowCount * 2, 3 * steps + 3 + 1);
...
for s = 1:maxNumIter
  ...
  for i = 2:1 + steps
    ...
    for j = 1:rows(l)
      if r(i, j) > 0
        % some pre-computations
        a1 = l(j, 1) - x_est(i, 1) - d_est * ct1;
        b1 = l(j, 2) - x_est(i, 2) - d_est * st1;
        temp1 = a1^2 + b1^2;
        temp2 = sqrt(temp1);
