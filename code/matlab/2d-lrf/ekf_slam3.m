...
% 3 parameters are considered as given.
...
for i = 1:rows(l)
  l_init(i, 1) = x_map_init(1) + d_x * cos(x_map_init(3)) - d_y *...
    sin(x_map_init(3)) + r(2, i) * cos(x_map_init(3) + phi + b(2, i));
  l_init(i, 2) = x_map_init(2) + d_x * sin(x_map_init(3)) + d_y *...
    cos(x_map_init(3)) + r(2, i) * sin(x_map_init(3) + phi + b(2, i));
end
...
for k = 2:steps
  ...
  if L_k > 0
    ...
    for j = 1:rows(l)
      if r(k, j) > 0
        % some pre-computations
        a1 = xm_k(4 + (j - 1) * 2) - xm_k(1) - d_x * ct1 + d_y * st1;
        b1 = xm_k(4 + (j - 1) * 2 + 1) - xm_k(2) - d_x * st1 - d_y * ct1;
        temp1 = a1^2 + b1^2;
        temp2 = sqrt(temp1);
        ...
        g(1) = temp2;
        g(2) = atan2(b1, a1) - xm_k(3) - phi;
        g_k(idx:idx + 1) = g;

        % jacobian of observation model with respect to state variables
        G_x(1, 1) = -a1 / temp2;
        G_x(1, 2) = -b1 / temp2;
        G_x(1, 3) = (a1 * (d_x * st1 + d_y * ct1) + b1 *...
          (-d_x * ct1 + d_y * st1)) / temp2;
        G_x(2, 1) = b1 / temp1;
        G_x(2, 2) = -a1 / temp1;
        G_x(2, 3) = (a1 * (-d_x * ct1 + d_y * st1) - b1 *...
          (d_x * st1 + d_y * ct1)) / temp1 - 1;
