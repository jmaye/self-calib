...
% the iterative algorithm we need.
...
for i = 2:1000
  ...
  if mod(i, batchSize) == 0
    ...
    H = sparse(3 * (batchSize - 1) * length(batchIndices) + obsCount * 2, ...
      3 * batchSize * length(batchIndices) + 2 * rows(l_est_temp) + 3);
    ...
    for s = 1:maxNumIter
      ...
      for j = 1:length(batchIndices)
        ...
        for k = batchIndices(j) - batchSize + 2:batchIndices(j)
          ...
          for m = 1:rows(l_est_temp)
            if r(k, m) > 0
              % some pre-computations
              a1 = l_est_temp(m, 1) - x_est_temp(n, 1) - d_x_est_temp * ct1 +...
                d_y_est_temp * st1;
              b1 = l_est_temp(m, 2) - x_est_temp(n, 2) - d_x_est_temp * st1 -...
                d_y_est_temp * ct1;
              temp1 = a1^2 + b1^2;
              temp2 = sqrt(temp1);
      ...
      l_update = [dx(temp + 1:2:end - 3) dx(temp + 2:2:end - 3)];
