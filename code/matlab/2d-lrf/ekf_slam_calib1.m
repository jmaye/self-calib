...
% This script is performing SLAM and calibration of a laser range finder offset
% based on Tim Barfoot's example using an Extended Kalman Filter (EKF).
...
for i = 1:rows(l)
  l_init(i, 1) = x_map_init(1) + d_init * cos(x_map_init(3)) + r(2, i) *...
    cos(x_map_init(3) + b(2, i));
  l_init(i, 2) = x_map_init(2) + d_init * sin(x_map_init(3)) + r(2, i) *...
...
% filtered state
x = zeros(steps, 3 + rows(l) * 2 + 1);
x(1, :) = [x_true(1), y_true(1), th_true(1), ...
  reshape(l_init', rows(l) * cols(l), 1)', d_init];

% filtered covariance
P = zeros(3 + rows(l) * 2 + 1, 3 + rows(l) * 2 + 1, steps);
P(1:3, 1:3, 1) = diag([1, 1, 0.1]);
P(4:end - 1, 4:end - 1, 1) = eye(rows(l) * 2, rows(l) * 2) * 0.01;
P(end, end, 1) = 0.01;
...
for k = 2:steps
  ...
  Pm_kRM = H_x * P(1:3, 4:end - 1, k - 1);
  Pm_kRC = H_x * P(1:3, end, k - 1);
  Pm_k = P(:, :, k - 1);
  Pm_k(1:3, 1:3) = Pm_kRR;
  Pm_k(1:3, 4:end - 1) = Pm_kRM;
  Pm_k(4:end - 1, 1:3) = Pm_kRM';
  Pm_k(1:3, end) = Pm_kRC;
  Pm_k(end, 1:3) = Pm_kRC';
  ...
  if L_k > 0
    ...
    for j = 1:rows(l)
      if r(k, j) > 0
        % some pre-computations
        a1 = xm_k(4 + (j - 1) * 2) - xm_k(1) - xm_k(end) * ct1;
        b1 = xm_k(4 + (j - 1) * 2 + 1) - xm_k(2) - xm_k(end) * st1;
        temp1 = a1^2 + b1^2;
        temp2 = sqrt(temp1);
        ...
        G_x(1, 1) = -a1 / temp2;
        G_x(1, 2) = -b1 / temp2;
        G_x(1, 3) = (a1 * xm_k(end) * st1 - b1 * xm_k(end) * ct1) / temp2;
        G_x(2, 1) = b1 / temp1;
        G_x(2, 2) = -a1 / temp1;
        G_x(2, 3) = -(a1 * xm_k(end) * ct1 + b1 * xm_k(end) * st1) / temp1 - 1;
