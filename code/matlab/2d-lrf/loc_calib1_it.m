...
% using based on Tim Barfoot's example. This is the definitive version of the
% iterative algorithm we need, but with known landmark's poses.
...
for i = 2:1000
  ...
  if mod(i, batchSize) == 0
    ...
    H = sparse(3 * (batchSize - 1) * length(batchIndices) + obsCount * 2, ...
      3 * batchSize * length(batchIndices) + 1);
      ...
      for j = 1:length(batchIndices)
        ...
        for k = batchIndices(j) - batchSize + 2:batchIndices(j)
          ...
          for m = 1:rows(l)
            if r(k, m) > 0
              % some pre-computations
              a1 = l(m, 1) - x_est_temp(n, 1) - d_est_temp * ct1;
              b1 = l(m, 2) - x_est_temp(n, 2) - d_est_temp * st1;
              temp1 = a1^2 + b1^2;
              temp2 = sqrt(temp1);
