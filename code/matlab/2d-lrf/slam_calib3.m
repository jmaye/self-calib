...
% parameters based on Tim Barfoot's example.
...
% matrices allocation
% jacobian matrix
H = sparse(3 * steps + rowCount * 2, 3 * steps + 3 + 2 * rows(l_est) + 3);
...
for s = 1:maxNumIter
  ...
  for i = 2:1 + steps
    ...
    for j = 1:rows(l_est)
      if r(i, j) > 0
        % some pre-computations
        a1 = l_est(j, 1) - x_est(i, 1) - d_x_est * ct1 + d_y_est * st1;
        b1 = l_est(j, 2) - x_est(i, 2) - d_x_est * st1 - d_y_est * ct1;
        temp1 = a1^2 + b1^2;
        temp2 = sqrt(temp1);
  ...
  l_update = [dx(3 * steps + 3 + 1:2:end - 3) dx(3 * steps + 3 + 2:2:end - 3)]
