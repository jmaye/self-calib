...
% with three calibration parameters.
...
% ground truth offset in x for laser range finder
d_x = 0.2190;

% ground truth offset in y for laser range finder
d_y = 0.1;

% ground truth angle for laser range finder
phi = pi / 4;
...
for i = 2:steps
  ...
  st = sin(th_true(i));
  for j = 1:nl
    a1 = l(j, 1) - x_true(i) - d_x * ct + d_y * st;
    b1 = l(j, 2) - y_true(i) - d_x * st - d_y * ct;
    r(i, j) = sqrt(a1^2 + b1^2) + randn * sqrt(r_var);
    b(i, j) = atan2(b1, a1) - th_true(i) - phi + randn * sqrt(b_var);
