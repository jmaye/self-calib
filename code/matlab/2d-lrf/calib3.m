...
% robot's poses and landmark' positions are not estimated here.
...
for s = 1:maxNumIter
  ...
  for i = 1:steps
    ...
    for j = 1:rows(l)
      if r(i, j) > 0
        % some pre-computations
        a1 = l(j, 1) - x_true(i) - d_x_est * ct1 + d_y_est * st1;
        b1 = l(j, 2) - y_true(i) - d_x_est * st1 - d_y_est * ct1;
        temp1 = a1^2 + b1^2;
        temp2 = sqrt(temp1);
        ...
        e(row) = r(i, j) - temp2;
        e(row + 1) = b(i, j) - (atan2(b1, a1) - th_true(i) - phi_est);
  ...
  % update estimate
  d_x_est = d_x_est + dx(1);
  d_y_est = d_y_est + dx(2);
  phi_est = phi_est + dx(3);
