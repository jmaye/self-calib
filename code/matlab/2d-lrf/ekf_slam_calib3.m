...
% This script is performing SLAM and calibration of a laser range finder with
% 3 parameters based on Tim Barfoot's example using an Extended Kalman Filter
% (EKF).
...
d_x_init = d_x + randn * 0.1
d_y_init = d_y + randn * 0.1
phi_init = phi + randn * 0.1
...
for i = 1:rows(l)
  l_init(i, 1) = x_map_init(1) + d_x_init * cos(x_map_init(3)) - d_y_init *...
    sin(x_map_init(3)) + r(2, i) * cos(x_map_init(3) + phi_init + b(2, i));
  l_init(i, 2) = x_map_init(2) + d_x_init * sin(x_map_init(3)) + d_y_init *...
    cos(x_map_init(3)) + r(2, i) * sin(x_map_init(3) + phi_init + b(2, i));
end
...
% filtered state
x = zeros(steps, 3 + rows(l) * 2 + 3);
x(1, :) = [x_true(1), y_true(1), th_true(1), ...
  reshape(l_init', rows(l) * cols(l), 1)', d_x_init, d_y_init, phi_init];

% filtered covariance
P = zeros(3 + rows(l) * 2 + 3, 3 + rows(l) * 2 + 3, steps);
P(1:3, 1:3, 1) = diag([1, 1, 0.1]);
P(4:end - 3, 4:end - 3, 1) = eye(rows(l) * 2, rows(l) * 2) * 0.01;
P(end - 2, end - 2, 1) = 0.01;
P(end - 1, end - 1, 1) = 0.01;
P(end, end, 1) = 0.01;
...
for k = 2:steps
  ...
  Pm_kRM = H_x * P(1:3, 4:end - 3, k - 1);
  Pm_kRC = H_x * P(1:3, end - 2:end, k - 1);
  Pm_k = P(:, :, k - 1);
  Pm_k(1:3, 1:3) = Pm_kRR;
  Pm_k(1:3, 4:end - 3) = Pm_kRM;
  Pm_k(4:end - 3, 1:3) = Pm_kRM';
  Pm_k(1:3, end - 2:end) = Pm_kRC;
  Pm_k(end - 2:end, 1:3) = Pm_kRC';
  ...
  if L_k > 0
    ...
    for j = 1:rows(l)
      if r(k, j) > 0
        % some pre-computations
        a1 = xm_k(4 + (j - 1) * 2) - xm_k(1) - xm_k(end - 2) *...
          ct1 + xm_k(end - 1) * st1;
        b1 = xm_k(4 + (j - 1) * 2 + 1) - xm_k(2) - xm_k(end - 2) *...
          st1 - xm_k(end - 1) * ct1;
        temp1 = a1^2 + b1^2;
        temp2 = sqrt(temp1);
        ...
        g(1) = temp2;
        g(2) = atan2(b1, a1) - xm_k(3) - xm_k(end);
        g_k(idx:idx + 1) = g;

        % jacobian of observation model with respect to state variables
        G_x(1, 1) = -a1 / temp2;
        G_x(1, 2) = -b1 / temp2;
        G_x(1, 3) = (a1 * (xm_k(end - 2) * st1 + xm_k(end - 1) * ct1) + b1 *...
          (-xm_k(end - 2) * ct1 + xm_k(end - 1) * st1)) / temp2;
        G_x(2, 1) = b1 / temp1;
        G_x(2, 2) = -a1 / temp1;
        G_x(2, 3) = (a1 * (-xm_k(end - 2) * ct1 + xm_k(end - 1) * st1) - b1 *...
          (xm_k(end - 2) * st1 + xm_k(end - 1) * ct1)) / temp1 - 1;
