...
% based on Tim Barfoot's example.
...
% matrices allocation
% jacobian matrix
H = sparse(3 * steps + rowCount * 2 + 1, 3 * steps + 3 + 2 * rows(l_est) + 1);

% error terms
e = zeros(3 * steps + rowCount * 2 + 1, 1);
...
for s = 1:maxNumIter
  ...
  for i = 2:1 + steps
    ...
    for j = 1:rows(l_est)
      if r(i, j) > 0
        % some pre-computations
        a1 = l_est(j, 1) - x_est(i, 1) - d_est * ct1;
        b1 = l_est(j, 2) - x_est(i, 2) - d_est * st1;
        temp1 = a1^2 + b1^2;
        temp2 = sqrt(temp1);
        ...
        G_x(1, 1) = -a1 / temp2;
        G_x(1, 2) = -b1 / temp2;
        G_x(1, 3) = (a1 * d_est * st1 - b1 * d_est * ct1) / temp2;
        G_x(2, 1) = b1 / temp1;
        G_x(2, 2) = -a1 / temp1;
        G_x(2, 3) = -(a1 * d_est * ct1 + b1 * d_est * st1) / temp1 - 1;
  ...
  l_update = [dx(3 * steps + 3 + 1:2:end - 1) dx(3 * steps + 3 + 2:2:end - 1)]
  l_est = l_est + l_update;
  d_update = dx(end)
  d_est = d_est + d_update
