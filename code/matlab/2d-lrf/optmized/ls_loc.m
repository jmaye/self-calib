...
% This function performs least squares localization for a robot with a laser
% range finder with known landmark positions and calibration parameters.

function est = ls_loc(l, Theta, x_hat, u, r, b, t, Q, R, maxIter, optTol, ...
  rankTol)
    ...
    for j = 1:nl
      if r(i, j) > 0
        ...
        if numCalib < 3
          dct = Theta(1) * ct1;
          dst = Theta(1) * st1;
          aa = l(j, 1) - est(i, 1) - dct;
          bb = l(j, 2) - est(i, 2) - dst;
        else
          dxct = Theta(1) * ct1;
          dxst = Theta(1) * st1;
          dyct = Theta(2) * ct1;
          dyst = Theta(2) * st1;
          aa = l(j, 1) - est(i, 1) - dxct + dyst;
          bb = l(j, 2) - est(i, 2) - dxst - dyct;
        end
        temp1 = aa^2 + bb^2;
        temp2 = sqrt(temp1);
