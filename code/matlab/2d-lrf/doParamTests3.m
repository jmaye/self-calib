...
for amplitude = minAmplitude:amplitudeStep:maxAmplitude
  ...
  for rankTol = minRankTol:rankTolStep:maxRankTol
    ...
    for batchSize = 10:50:300
    for miThreshold = 0.1:0.2:1
    ...
    paramResTQRMIAll(ampIdx).amplitude = amplitude;
    paramResTQRMIAll(ampIdx).rankTol = rankTol;
    paramResTQRMIAll(ampIdx).batchSize = batchSize;
    paramResTQRMIAll(ampIdx).miThreshold = miThreshold;
    paramResTQRMIAll(ampIdx).rep = rep;
    paramResTQRMIAll(ampIdx).Theta_tqr_mi = Theta_tqr_mi;
    paramResTQRMIAll(ampIdx).Sigma_tqr_mi = Sigma_tqr_mi;
    paramResTQRMIAll(ampIdx).batchSize_rep = batchSize_rep;

    % save file to disk
    save('paramResTQRMIAll.mat', 'paramResTQRMIAll');
